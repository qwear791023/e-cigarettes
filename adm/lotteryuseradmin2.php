...
if($_POST['id']){
    $id = (int) $_POST['id'];
    $sql = "delete from `lottery_user2` where `id`=$id";
    $rs = $db->query($sql);
    $res=array('status'=>false,'msg'=>'delete failured');
    if($rs){
        $res['status'] = true;
        $res['msg']="SUCCESS!!";
    }
...
$smarty->assign('nav',2);
$sql="select type,count(*) as `total` from `lottery_user2` group by `type`";
$sList=$db->get_results($sql);
$smarty->assign('sList',$sList);
$sql="SELECT  SQL_CALC_FOUND_ROWS * FROM `lottery_user2`  ORDER BY `id` DESC LIMIT ".$perpage*($page-1).",$perpage ";
...
$smarty->display("adm/lotteryuseradmin2.html");
