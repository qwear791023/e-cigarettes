...
if(!$page)
	$page=1;
$sql="SELECT BLOG_ID,count(*) as TOTAL FROM `FB_BLOG_SHARE` group by BLOG_ID";
 $itemList=$db->get_results($sql);
 $smarty->assign('sitemList',$itemList);

$smarty->assign('nav',8);
$sql="SELECT  SQL_CALC_FOUND_ROWS * FROM `FB_BLOG_SHARE`  ORDER BY FBS_ID DESC LIMIT ".$perpage*($page-1).",$perpage ";
$itemList=$db->get_results($sql);
...
$smarty->display("adm/tuseradmin.html");
